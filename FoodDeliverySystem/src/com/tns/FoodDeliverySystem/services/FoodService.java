...
public class FoodService {
 ...
 // Method to add a restaurant
    ...
    // Method to get a food item from a specific restaurant by ID
    public FoodItem getFoodItemFromRestaurant(int restaurantId, int foodItemId) {
        for (Restaurant restaurant : restaurants) {
            if (restaurant.getId() == restaurantId) {
                for (FoodItem foodItem : restaurant.getMenu()) {
                    if (foodItem.getId() == foodItemId) {
                        return foodItem;  // Found the food item
                    }
                }
                System.out.println("Food item with ID " + foodItemId + " not found in restaurant " + restaurant.getName());
                return null;
            }
        }
        System.out.println("Restaurant with ID " + restaurantId + " not found.");
